            ...
            if origem_tipo == "OM":
                ...
                om_linhas = conn.execute(text("""
                    SELECT
                        ...
                        UPPER(COALESCE(tipo_linha, descricao, detalhes, '')) AS detalhe,
                        COALESCE(valor_brl, 0) AS valor_ref,
                        UPPER(COALESCE(:numero_origem, '')) AS origem_ref,
                        '--' AS controle_ref,
                ...
                rd_linhas = conn.execute(text("""
                    SELECT
                        id,
                        TO_CHAR(data_lancamento, 'DD/MM/YYYY') AS data_ref,
                        UPPER(COALESCE(descricao, detalhes, '')) AS detalhe,
                        COALESCE(valor, 0) AS valor_ref,
                        UPPER(COALESCE(:numero_origem, '')) AS origem_ref,
                        '--' AS controle_ref,
                        UPPER(COALESCE(categoria, aplicacao, :cc)) AS cc_ref,
                        id AS linha_ref,
                        '' AS recibo_ref
                    FROM financeiro2_rd_linhas
                    WHERE rd_id = :origem_id
                ...
                totais = {
                    "qtd_vinculadas": 1 if item["status_nd"] == "VINCULADA" else 0,
                    "qtd_desconsideradas": 1 if item["status_nd"] == "DESCONSIDERADA" else 0,
                    "valor_vinculado": float(item["valor_operacional"] or 0) if item["status_nd"] == "VINCULADA" else 0.0,
                    "valor_desconsiderado": 0.0,
                    "valor_pendente": float(item["valor_operacional"] or 0) if item["status_nd"] in ("NÃO VINCULADA", "PARCIAL") else 0.0,
                }
...
@bp.route("/despesas/<int:despesa_id>/desvincular")
@login_required
@permission_required("financeiro", "visualizar")
def despesa_desvincular(despesa_id: int):
    if not _usuario_eh_administrador():
        flash("APENAS O PERFIL ADMINISTRADOR PODE DESVINCULAR DESPESA OPERACIONAL.", "danger")
        return redirect(url_for("financeiro_dois.despesa_editar", despesa_id=despesa_id))
    ...
    with engine.connect() as conn:
        ...
        if despesa["origem_tipo"] != "OPERACIONAL":
            flash("APENAS DESPESAS OPERACIONAIS PODEM SER DESVINCULADAS NESTA TELA.", "warning")
            return redirect(url_for("financeiro_dois.despesa_editar", despesa_id=despesa_id))
    ...
    return render_template(
        "financeiro_dois/despesa_desvincular.html",
        subnav_links=build_financeiro_dois_subnav("despesas"),
        despesa=despesa,
    )
    
@bp.route("/despesas/<int:despesa_id>/desvincular/salvar", methods=["POST"])
@login_required
@permission_required("financeiro", "visualizar")
def despesa_desvincular_salvar(despesa_id: int):
    if not _usuario_eh_administrador():
        flash("APENAS O PERFIL ADMINISTRADOR PODE DESVINCULAR DESPESA OPERACIONAL.", "danger")
        return redirect(url_for("financeiro_dois.despesa_editar", despesa_id=despesa_id))
    ...
    senha = _nome_preenchido(request.form.get("senha_confirmacao"))
    if not senha:
        flash("INFORME A SENHA DO USUÁRIO.", "warning")
        return redirect(url_for("financeiro_dois.despesa_desvincular", despesa_id=despesa_id))
